...
export class Fetches {
    ...
    public static async TestServerLink(): Promise<any | Error> {
        try {
            const res = await axios.get(Requests.TestServerLink)
            if (res.status !== 200) {
                return Error("ошибка")
            }
            console.log(res)
    ...
    public static async GetAuthStatus(): Promise<AuthStatus | Error> {
        try {
            const res = await axios.get<AuthStatus>(Requests.GetAuthStatus)
            if (res.status !== 200) {
                return Error("ошибка")
            }
            return res.data
